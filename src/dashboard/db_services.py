# Arquivo: src/dashboard/db_services.py
import sqlite3
import json
import pandas as pd
...
def obter_lista_deputados():
    ...
    try:
        query = "SELECT nome || ' (' || partido || '-' || uf || ')' AS nome_completo, nome FROM deputados ORDER BY nome"
...
def carregar_auditoria_gold(nome_deputado):
    ...
    try:
        ...
        votos_count = cursor.execute("SELECT COUNT(*) FROM votos_gold_youtube WHERE UPPER(nome_deputado) LIKE ?", (f"%{nome_deputado.upper()}%",)).fetchone()[0]
        ...
        if votos_count > 0:
            ...
            try:
                conf_row = cursor.execute("SELECT taxa_confianca FROM votos_gold_youtube WHERE UPPER(nome_deputado) LIKE ?", (f"%{nome_deputado.upper()}%",)).fetchone()
