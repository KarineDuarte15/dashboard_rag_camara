# Arquivo: src/camara/cruzamento_dados.py
import sqlite3
import pandas as pd
import warnings
...
def processar_cruzamento(nome_deputado_input, pergunta_usuario, temas_identificados, id_pergunta, nome_banco='camara_dados.db'):
    try:
        conn = sqlite3.connect(nome_banco)
        # UPPER() nativo do SQLite não maiúsculiza acentos (só ASCII);
        # substitui por versão Python ciente de Unicode.
    ...
    if df_temas_filtrado.empty:
        print("⚠️ Nenhuma proposição encontrada com os temas selecionados pela IA.")
        conn.close()
        return False
    ...
    query_votos = f"SELECT id_votacao, `deputado_.id`, `deputado_.nome` AS nome_deputado, tipoVoto FROM votos_deputados WHERE UPPER(`deputado_.nome`) LIKE '%{nome_deputado_input}%'"
    df_votos = pd.read_sql_query(query_votos, conn)

    if df_votos.empty:
        print(f"⚠️ Não encontramos votos para o deputado: {nome_deputado_input}")
        conn.close()
        return False
    ...
    df_final['id_pergunta'] = id_pergunta

    colunas_ordenadas = [
        'id_proposicao', 'ementa', 'tema', 'id_votacao', 'aprovacao', 
        'deputado_.id', 'nome_deputado', 'tipoVoto', 'pergunta_usuario', 'id_pergunta'
    ]
    ...
    if not df_final.empty:
        ...
        print(f"✅ Tabela Staging '{nome_tabela_temp}' gerada com {len(df_final)} registros.")
        retorno = True
    else:
        print("⚠️ Nenhum cruzamento encontrado para os temas.")
        retorno = False
